...
` */
  ...
  */
  ...
  function toTitleCase(x) {
    ...
    x = x.replace(/['"]?[a-z]['"]?(?= )/g, function(match){ 
      console.log(match);
    ...
    x = x.replace(/ .*?(?= )/g, function(match){
      ...
      // } else if (isAllCaps(removeSymbols(match))){
      //   return " " + toSentenceCase(match.toLowerCase().trim());
      // }
      return match;
    });

    // replace all caps words
    // var z = x.split(" ").map(function(d){ return isAllCaps(d) ? toSentenceCase(d) : d; }).join(" ");
    // console.log(z);
